...
箱中节点的频率遵循泊松分布
（http:en.wikipedia.orgwikiPoisson_distribution）
给定大小调整阈值，参数平均约为0.5
为0.75，但由于大小调整，差异较大
粒度。忽略方差，则
...
类别和识别代码为联络断路器。红黑
平衡代码是从jdk之前的集合中更新的
（http:gee.cs.oswego.edudlclassecollectionsrbcell.java）
依次基于Cormen、Leiserson和Rivest的《关于
算法”（CLR）。树蛋白还需要额外的锁定机制。虽然
读者即使在阅读过程中也可以遍历列表
更新时，树遍历不是，主要是因为树旋转
这可能会改变根节点和或其连接。树蜘蛛
包括寄生在主机上的简单读写锁定机制
...
//    1073741824
    ...
    //这里的意思是，通过亦或操作混合高位和低位的值，并且把最为高位（符号位）强制为0（正数）
    /**
    ...
    /*
        易失性访问方法用于表元素以及
        调整大小时，下一个表中正在进行的元素。所有用途
        调用者必须检查选项卡参数是否为空。所有来电者
        此外，还要偏执地预先检查选项卡的长度是否不是零（或
        等价检查），从而确保任何索引参数
        哈希值与（长度-1）AND的形式是有效的
        指数请注意，要更正wrt任意并发
        如果用户出错，这些检查必须对局部变量进行操作，
        这就解释了下面一些看起来很奇怪的内联作业。
        请注意，对setTabAt的调用总是发生在锁定区域内，
        所以原则上只需要发布命令，而不是
        完整的volatile语义，但目前编码为volatile
        写文章要保守。
     */
    ...
    final V putVal(K key, V value, boolean onlyIfAbsent) {
        ...
        //融合hashcode高16位和低16位的值，使得高位在数组较小的时候高位可以参与计算减小碰撞发生的概率，并强制为正数
