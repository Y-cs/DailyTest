...
import timer.main.exception.LimitingCheckException;
import timer.main.exception.LimitingCreateException;
import timer.main.exception.LimitingGroupNotFoundException;
import timer.main.support.LimitingSupport;
import timer.main.util.CacheNameUtil;
...
public class OperationLimiting {
    ...
    private boolean checkByClass(Class<?> clazz, LimitingGroup limitingGroup, Limiting limiting, HttpServletRequest httpServletRequest) throws LimitingCreateException {
        ...
        group = group != null && group.length() > 0 ? group : CacheNameUtil.getDefaultName(clazz, null, limitingGroup.partitionEnum());
    ...
    private boolean checkByGroup(Class<?> clazz, MethodSignature methodSignature, Limiting limiting, HttpServletRequest httpServletRequest) throws LimitingCheckException, LimitingCreateException {
        ...
        for (LimitingGroup limitingGroup : group) {
            ...
            //组名
            groupName = limitingGroup.group() == null || limitingGroup.group().trim().length() == 0 ? CacheNameUtil.getDefaultName(clazz, methodSignature.getMethod(), limitingGroup.partitionEnum()) : limitingGroup.group();
            //根据组名获取或注册一个限流器
